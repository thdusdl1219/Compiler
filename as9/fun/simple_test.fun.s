...
_main:
	...
	move 	$a0, $v0	# $a0 := $v0
	beqz	$a0, L5   	# if (signed) $a0 == 0 goto L5
	li 	$a0, 7		# $a0 := 7
	seq	$a0, $s0, $a0	# $a0 := $s0==$a0
	li 	$s3, 0		# $s3 := 0
	seq	$a0, $a0, $s3	# $a0 := $a0==$s3
	beqz	$a0, L7   	# if (signed) $a0 == 0 goto L7
	la 	$s3, _addsome	# $s3 := _addsome
	la 	$s4, _add	# $s4 := _add
	li 	$a0, 8		# $a0 := 8
	jal 	alloc		# call alloc
	move 	$a0, $v0	# $a0 := $v0
	...
	sw 	$s1, 4($a0)	# [$a0+4] := $s1
				# $a0 := $a0
	jalr 	$ra, $s4	# ($a0) := call $ra($v0,$ra,$a0,$a1,$a2,$a3,$t0,$t1,$t2,$t3,$t4,$t5,$t6,$t7,$t8,$t9,$v0,$v1)
	move 	$a0, $v0	# $a0 := $v0
				# $a0 := $a0
	jalr 	$ra, $s3	# ($a0) := call $ra($v0,$ra,$a0,$a1,$a2,$a3,$t0,$t1,$t2,$t3,$t4,$t5,$t6,$t7,$t8,$t9,$v0,$v1)
	move 	$a0, $v0	# $a0 := $v0
