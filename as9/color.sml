...
struct
 ...
 val coalT : M.reg RT.table ref = ref RT.empty

 fun coalsce(movegraph, ig, ori_ig) =
  let 
    fun adj reg : RS.set = IG.adj ig reg;
    fun redgs r1 r2 : unit = IG.rm_edge ig {from = r1, to = r2};
  ...
  in
    ...
    (*                remove_edge(adj, redgs) reg;
                    remove_edge(ori_adj, ori_redgs) reg; *)
 ...
 fun makeLowdegsHighdegs (adj : M.reg -> RS.set, nodes : unit -> RS.set, palette
   : RS.set) table : {lowdegs : RS.set, highdegs : RS.set} = 
   let
 ...
 fun simplify (lowdegs : RS.set, stack : M.reg list, adj : M.reg -> RS.set,
   redgs : M.reg -> M.reg -> unit) table : M.reg list =
 ...
 fun spillF (highdegs : RS.set, spill : RS.set, adj : M.reg -> RS.set, redgs :
   M.reg -> M.reg -> unit, spillCost : M.reg -> int) table : RS.set =
    let
    ...
    in
      ...
      RS.add(spill, reg)
      (*RS.foldl (fn (reg, set) => 
                  if (RS.member(set, getalias table reg)) 
                  then RS.add(set, reg)
                  else set
                ) (RS.add(spill, reg)) (RS.delete(highdegs, reg))*)
 ...
 fun coloringFunction (lowdegs : RS.set, highdegs: RS.set, org_ig : IG.graph, ig
  ...
  in
   case (RS.numItems(lowdegs), RS.numItems(highdegs)) of
      ...
      spillCost) table; val lowdegs_highdegs = makeLowdegsHighdegs (adj, nodes,
      palette) table in 
        (coloringFunction (#lowdegs(lowdegs_highdegs),
        #highdegs(lowdegs_highdegs), org_ig, ig, stack, spill, palette,
        spillCost) table) end

      | (_, _) => let val stack = simplify (lowdegs,stack,adj, redgs) table; val
      lowdegs_highdegs = makeLowdegsHighdegs (adj, nodes, palette) table in 
        coloringFunction (#lowdegs(lowdegs_highdegs),
        #highdegs(lowdegs_highdegs), org_ig, ig, stack, spill, palette,
        spillCost) table end
  end
 ...
 fun color ({interference = ig: IG.graph,
             ...
             palette: RS.set}) : coloring = let 
               ...
               val _ = coalT := coalsce (moves, ig, org_ig)
               val lowdegs_highdegs = makeLowdegsHighdegs ((fn r => IG.adj ig
               r), (fn () => IG.nodes ig), palette) (!coalT)
