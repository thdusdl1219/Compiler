...
2. Open sml console and type "CM.make "heosy.cm";"
3. Heosy.test() to test with all test suites
...
struct
  ...
  fun funfile () = 
  let val files = ["simple-identity.fun", "test.fun", "all.fun"] 
  ...
  fun checklist () = 
  let val files = [("error-main.fun", false), ("error-duplicate.fun", false), ("silly.fun", true)]
  in case List.filter (fn (f, r) => r <> (filetest f)) files of
       nil => true
     | errs => 
       let val errmsg = List.map (fn (f, r) => f) errs
       in (print ("\nchecklist: failed to check type of "^(String.concatWith ", " errmsg)); false)
       end
 end
